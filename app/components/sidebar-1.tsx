...
export default function Sidebar() {
  ...
  return (
    <div className={`relative h-full bg-white text-black transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-16'} flex-shrink-0 shadow-lg z-10`}>
      <nav className="mt-6">
        {sidebarItems.map((item, index) => {
          const isActive = pathname === item.href
          return (
            <Link key={index} href={item.href}>
              <div className={`flex items-center p-4 ${isOpen ? 'hover:bg-purple-100' : ''}`}>
                <div className={`w-8 h-8 flex items-center justify-center ${isActive 
                  ? isOpen 
                    ? 'bg-purple-100 rounded-md' 
                    : 'bg-purple-100 rounded-full'
                  : ''
                }`}>
                  <item.icon size={20} color={isActive ? '#8B5CF6' : '#000000'} />
                </div>
                {isOpen && (
                  <span className={`ml-4 transition-opacity duration-300 ${isActive ? 'text-purple-600 font-semibold' : ''}`}>
                    {item.text}
                  </span>
                )}
              </div>
      ...
      <button 
        className="absolute top-[50px] -right-4 bg-white text-black border border-gray-200 rounded-full p-1 hover:bg-gray-100 focus:outline-none shadow-md z-20"
        onClick={() => setIsOpen(!isOpen)}
      >
        {isOpen ? <FiChevronLeft size={20} /> : <FiChevronRight size={20} />}
      </button>
